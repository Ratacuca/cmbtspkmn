...
alter table COMBATE add constraint FK_COMPITEN_MEDIANTE foreign key (ID_LIGAPOKEMON)
      references LIGA_POKEMON (ID_LIGAPOKEMON) on delete restrict on update restrict;

alter table COMBATE add constraint FK_CONTRINCANTE foreign key (ID_ENTRENADOR)
      references ENTRENADOR (ID_ENTRENADOR) on delete restrict on update restrict;

alter table COMBATE add constraint FK_ESTA_COMPUESTA foreign key (ID_TORREBATALLA)
      references TORRE_BATALLA (ID_TORREBATALLA) on delete restrict on update restrict;

alter table COMBATE add constraint FK_RETADOR foreign key (ENT_ID_ENTRENADOR)
      references ENTRENADOR (ID_ENTRENADOR) on delete restrict on update restrict;

alter table ELEMENTOS_POKEMON add constraint FK_ES_DE foreign key (ID_FAMILIAPOKEMON)
      references FAMILIA_POKEMON (ID_FAMILIAPOKEMON) on delete restrict on update restrict;

alter table ELEMENTOS_POKEMON add constraint FK_SON foreign key (ID_ELEMENTO)
      references ELEMENTO (ID_ELEMENTO) on delete restrict on update restrict;

alter table ENTRENADOR add constraint FK_ENTREGA foreign key (ID_MEDALLA)
      references MEDALLAS (ID_MEDALLA) on delete restrict on update restrict;

alter table ENTRENADOR add constraint FK_NACEN foreign key (ID_REGION)
      references REGION (ID_REGION) on delete restrict on update restrict;

alter table EQUIPOPOKEMON add constraint FK_ENTRENA foreign key (ID_ENTRENADOR)
      references ENTRENADOR (ID_ENTRENADOR) on delete restrict on update restrict;

alter table EQUIPOPOKEMON add constraint FK_PELEA_POR foreign key (ID_FAMILIAPOKEMON, ID_POKEMON)
      references POKEMON (ID_FAMILIAPOKEMON, ID_POKEMON) on delete restrict on update restrict;

alter table HABILIDADPOKEMON_POKEMON add constraint FK_ES_UTILIZADA foreign key (ID_HABILIDADPOKEMON)
      references HABILIDADPOKEMON (ID_HABILIDADPOKEMON) on delete restrict on update restrict;

alter table HABILIDADPOKEMON_POKEMON add constraint FK_UTILIZA foreign key (ID_FAMILIAPOKEMON, ID_POKEMON)
      references POKEMON (ID_FAMILIAPOKEMON, ID_POKEMON) on delete restrict on update restrict;

alter table HISTORIAL_ENTRENADOR_TORRE_BATALLA add constraint FK_DESAFIA foreign key (ID_ENTRENADOR)
      references ENTRENADOR (ID_ENTRENADOR) on delete restrict on update restrict;

alter table HISTORIAL_ENTRENADOR_TORRE_BATALLA add constraint FK_ES_DESAFIADA foreign key (ID_TORREBATALLA)
      references TORRE_BATALLA (ID_TORREBATALLA) on delete restrict on update restrict;

alter table HISTRIAL_ENTRENADOR_LIGA_POKEMON add constraint FK_COMPITE foreign key (ID_ENTRENADOR)
      references ENTRENADOR (ID_ENTRENADOR) on delete restrict on update restrict;

alter table HISTRIAL_ENTRENADOR_LIGA_POKEMON add constraint FK_COMPITEN foreign key (ID_LIGAPOKEMON)
      references LIGA_POKEMON (ID_LIGAPOKEMON) on delete restrict on update restrict;

alter table MEDALLAS add constraint FK_ES_ENTREGADA foreign key (ID_REGION)
      references REGION (ID_REGION) on delete restrict on update restrict;

alter table MOVIMIENTO add constraint FK_ES_DE_UN foreign key (ID_ELEMENTO)
      references ELEMENTO (ID_ELEMENTO) on delete restrict on update restrict;

alter table MOVIMIENTOS_POSIBLES add constraint FK_PUEDE_APRENDER foreign key (ID_FAMILIAPOKEMON)
      references FAMILIA_POKEMON (ID_FAMILIAPOKEMON) on delete restrict on update restrict;

alter table MOVIMIENTOS_POSIBLES add constraint FK_PUEDE_SER_APRENDIDO foreign key (ID_MOVIMIENTO)
      references MOVIMIENTO (ID_MOVIMIENTO) on delete restrict on update restrict;

alter table MOVIMIENTO_APRENDIDO add constraint FK_APRENDE foreign key (POK_ID_FAMILIAPOKEMON, ID_POKEMON)
      references POKEMON (ID_FAMILIAPOKEMON, ID_POKEMON) on delete restrict on update restrict;

alter table MOVIMIENTO_APRENDIDO add constraint FK_ES_APRENDIDO foreign key (ID_MOVIMIENTO, ID_FAMILIAPOKEMON)
      references MOVIMIENTOS_POSIBLES (ID_MOVIMIENTO, ID_FAMILIAPOKEMON) on delete restrict on update restrict;

alter table NIVEL_DE_RESISTENCIA add constraint FK_ATACA foreign key (ID_ELEMENTODEFIENDE)
      references ELEMENTO (ID_ELEMENTO) on delete restrict on update restrict;

alter table NIVEL_DE_RESISTENCIA add constraint FK_DEFIENDE foreign key (ID_ELEMENTOATACA)
      references ELEMENTO (ID_ELEMENTO) on delete restrict on update restrict;

alter table POKEMON add constraint FK_PERTENECE foreign key (ID_FAMILIAPOKEMON)
      references FAMILIA_POKEMON (ID_FAMILIAPOKEMON) on delete restrict on update restrict;
